...
public class RSA {
	...
	public static RSAKey generateKeys (int bitLength, int seed) throws Exception {
		...
		//BigInteger e = new BigInteger((bitLength*bitLength)-2, 100, random);
		/*while (pq.gcd(e)!= one) {
			e = new BigInteger((bitLength*bitLength)-2, 100, random);
		}*/
		key.e = e;
		key.d = e.modInverse(pq);
	...
	public static CipherText encrypt (RSAKey key, String plainText) throws Exception {
		// your code here; a dummy return statement is put below to allow this to compile
		CipherText e = new CipherText();
		e.fileLength = plainText.length();
		e.blockLength = (key.n.intValue()-1)/8;
		String s= "";
		if (e.fileLength < e.blockLength) {
			s = plainText.substring(0,e.blockLength);
			e.encryptedBlocks.add(new BigInteger(s.getBytes()));
		}
		else {
			for (int i = 0; ((i+1)*e.blockLength) < plainText.length(); i++) {
				s = plainText.substring(i*e.blockLength, (i+1)*e.blockLength);
				e.encryptedBlocks.add(new BigInteger(s.getBytes()));
			}
			if ((e.fileLength%e.blockLength)!=0) {
				int b = (e.fileLength/e.blockLength);
				s = plainText.substring((b*e.blockLength), (e.fileLength));
				e.encryptedBlocks.add(new BigInteger(s.getBytes()));
			}
		}
		for (BigInteger b:e.encryptedBlocks) {
	...
	public static String decrypt (RSAKey key, CipherText cipherText) throws Exception {
		...
		String s = "";
		for (BigInteger b:cipherText.encryptedBlocks) {
			b = b.modPow(key.d, key.n);
			s = s + new String(b.toByteArray());
		}
		return s;
	}
	...
	public static void main (String[] args) throws Exception {
		/*String s = "this is beautiful and this is a test";
		RSAKey k = new RSAKey();
		k = generateKeys(4, 300);
		String d = decrypt(k,(encrypt(k,s)));
